...
import { VeeGPTTransitionProvider, VeeGPTRouteShell } from './features/veegpt/VeeGPTTransition';
import {
  Calendar,
  LayoutGrid,
  FileText,
...
const CreateDropdown = React.lazy(() =>
  import('./components/layout/create-dropdown').then(m => ({ default: m.CreateDropdown }))
);
const QuickActions = React.lazy(() =>
  import('./components/dashboard/quick-actions').then(m => ({ default: m.QuickActions }))
);
const PerformanceScore = React.lazy(() =>
  import('./components/dashboard/performance-score').then(m => ({ default: m.PerformanceScore }))
);
...
const GetStarted = React.lazy(() =>
  import('./components/dashboard/get-started').then(m => ({ default: m.GetStarted }))
);
const Listening = React.lazy(() =>
  import('./components/dashboard/listening').then(m => ({ default: m.Listening }))
);
const SocialAccounts = React.lazy(() =>
  import('./components/dashboard/social-accounts').then(m => ({ default: m.SocialAccounts }))
);
const InstagramWebhookListener = React.lazy(
  () => import('./components/dashboard/instagram-webhook-listener')
);
const ScheduledPostsSection = React.lazy(() =>
  import('./components/dashboard/scheduled-posts-section').then(m => ({
    default: m.ScheduledPostsSection,
  }))
);
const DraftsSection = React.lazy(() =>
  import('./components/dashboard/drafts-section').then(m => ({ default: m.DraftsSection }))
);
const BestTimeWidget = React.lazy(() =>
  import('./components/dashboard/best-time-widget').then(m => ({ default: m.BestTimeWidget }))
);
const CalendarView = React.lazy(() =>
  import('./components/calendar/calendar-view').then(m => ({ default: m.CalendarView }))
);
const PlanPageComponent = React.lazy(() =>
...
const AnalyticsApp = React.lazy(() =>
  import('./features/analytics').then(m => ({ default: m.AnalyticsApp }))
);
const CreatePost = React.lazy(() =>
  import('./components/create/create-post').then(m => ({ default: m.CreatePost }))
);
const PostAnalyticsPage = React.lazy(() => import('./pages/PostAnalyticsPage'));
const VeeGPT = React.lazy(() => import('./pages/VeeGPT'));
const AutoPilotPage = React.lazy(() =>
  import('./features/autopilot').then(m => ({ default: m.AutoPilotPage }))
);
...
const BillingHistoryPage = React.lazy(() => import('./pages/BillingHistoryPage'));
const CreditsPage = React.lazy(() => import('./pages/CreditsPage'));
const SubscriptionCheckoutPage = React.lazy(() =>
  import('./pages/SubscriptionCheckoutPage').then(m => ({ default: m.SubscriptionCheckoutPage }))
);
...
const SectionErrorBoundary = React.lazy(() =>
  import('./components/ErrorBoundary').then(m => ({ default: m.SectionErrorBoundary }))
);
const WorkspaceCreationOverlay = React.lazy(() => import('./components/WorkspaceCreationOverlay'));
const GuidedTour = React.lazy(() =>
  import('./components/walkthrough/GuidedTour').then(m => ({ default: m.GuidedTour }))
);
const PremiumSubscriptionModalHost = React.lazy(() =>
  import('./components/subscription/PremiumSubscriptionModalHost').then(m => ({
    default: m.PremiumSubscriptionModalHost,
  }))
);
const PaymentGraceModalHost = React.lazy(() =>
  import('./components/subscription/PaymentGraceModalHost').then(m => ({
    default: m.PaymentGraceModalHost,
  }))
);
const ModalPreviewPage = React.lazy(() => import('./pages/ModalPreviewPage'));
...
export default function AuthenticatedApp() {
  ...
  const { aiCredits, limits, plan: subscriptionPlan } = useSubscription();
  const availableCredits = aiCredits?.remaining;
  // Canonical plan label (the legacy userData.plan is stale/unreliable).
  ...
  // Social Listening is a Creator+ feature. When the plan doesn't include it we
  // HIDE the dashboard widget entirely (users only see features their plan
  // supports). `limits === undefined` means still loading — hide until known.
  const canUseSocialListening = limits?.features?.socialListening === true;
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => {
      try {
        (window as any).__vfRemoveShell?.('authenticatedapp-mount');
      } catch {
        /* ignore */
      }
    });
    ...
    if (!loading && !userDataLoading) {
      ...
      if (user && userData && (userData.isOnboarded || hasWorkspaces)) {
        ...
        if (!currentUrl.startsWith('/signup')) {
          console.log(
            '[ONBOARDING] User not onboarded, redirecting to signup to complete onboarding'
          );
      ...
      <LoadingStatusProvider>
       <VeeGPTTransitionProvider>
        ...
        <Switch location={location}>
          ...
          {/* Legacy combined "Content" page removed — the Plan workspace is now
              the single home for scheduled posts, drafts and published posts. */}
          <Route path="/posts">
            <Redirect to="/plan?tab=scheduled" />
          </Route>

          {/* Legacy standalone list pages removed — consolidated into the Plan workspace tabs. */}
          <Route path="/posts/scheduled">
            <Redirect to="/plan?tab=scheduled" />
          </Route>

          <Route path="/posts/drafts">
            <Redirect to="/plan?tab=drafts" />
          </Route>

          <Route path="/posts/published">
            <Redirect to="/plan?tab=published" />
          </Route>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        Craft content for Instagram
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-blue-50 dark:bg-blue-900/20 px-2.5 py-1 text-[11px] font-medium text-blue-700 dark:text-blue-300">
                      Instagram
                    </span>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                      <button className="px-3 py-1.5 rounded-md bg-white dark:bg-gray-700 shadow-sm text-xs font-semibold text-gray-900 dark:text-gray-100">
                        All
                      </button>
                      <button className="px-3 py-1.5 rounded-md text-xs font-medium text-gray-500 dark:text-gray-400">
                        Unread
                      </button>
                      <button className="px-3 py-1.5 rounded-md text-xs font-medium text-gray-500 dark:text-gray-400">
                        Mentions
                  ...
                  <div className="p-6 text-center py-12">
                    <p className="text-gray-600 dark:text-gray-400">
                      Manage your social media conversations here.
                    </p>
                  </div>
                </DashboardLayout>
              </ProtectedRoute>
          ...
          <Route path="/video-editor">
            <ProtectedRoute>
              <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex overflow-hidden relative transition-colors duration-300">
                ...
                <div className="flex-1 flex flex-col h-screen overflow-hidden">
                  {/* Video Editor header: title + icon */}
                  <div className="flex items-center justify-between px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-violet-50 dark:bg-violet-900/20">
                        <Video className="h-5 w-5 text-violet-600 dark:text-violet-400" />
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Edit video conversationally with AI
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <main className="flex-1 overflow-y-auto">
                    <React.Suspense fallback={<VideoEditorSkeleton />}>
                      <VideoEditorPage />
                    </React.Suspense>
                  </main>
                </div>
          ...
          {/* VeeGPT runs as its own "separate software" — the main app sidebar
              is intentionally NOT rendered here. VeeGPT provides its own shell
              (ConversationSidebar), and the VeeFore logo inside it returns to
              the main app. The launch/return animation is handled by
              VeeGPTTransitionProvider. */}
          <Route path="/veegpt">
            <ProtectedRoute>
          ...
          <Route path="/autopilot">
            <ProtectedRoute>
              <div className="h-screen w-full flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
                <React.Suspense
                  fallback={<div className="p-8 text-sm text-gray-500">Loading Auto Pilot…</div>}
                >
                  <AutoPilotPage />
                </React.Suspense>
              </div>
            ...
            <ProtectedRoute>
              ...
              >
                <div className="flex-1 overflow-y-auto">
                  <React.Suspense
                    fallback={
                      <div className="p-8 text-sm text-gray-500">Loading billing history…</div>
                    }
                  >
                    <BillingHistoryPage />
                  </React.Suspense>
          ...
          <Route path="/ai-usage">
            <ProtectedRoute>
              ...
              >
                ...
                <div className="flex items-center justify-between px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        Monitor credit consumption this month
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-medium text-gray-700 dark:text-gray-300">
                      {aiCredits?.usedThisCycle ?? 0} credits used
                    </p>
                ...
                <div className="p-6">
                  <React.Suspense
                    fallback={<div className="p-8 text-sm text-gray-500">Loading AI usage…</div>}
                  >
                    <AIUsageDashboard />
                  </React.Suspense>
                </div>
                ...
                <div className="flex items-center justify-between px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        AI credit balance and full transaction history
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-[11px] text-gray-400 dark:text-gray-500">
                      {aiCredits?.usedThisCycle ?? 0} used this cycle
                    </p>
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto">
                  <React.Suspense
                    fallback={<div className="p-8 text-sm text-gray-500">Loading credits…</div>}
                  >
                    <CreditsPage />
                  </React.Suspense>
                </div>
        ...
        {isWalkthroughOpen && (
          <React.Suspense fallback={null}>
            <GuidedTour isActive={isWalkthroughOpen} onClose={() => setIsWalkthroughOpen(false)} />
          </React.Suspense>
        )}
       </VeeGPTTransitionProvider>
      </LoadingStatusProvider>
