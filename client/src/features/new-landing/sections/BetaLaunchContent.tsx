...
import {
    Gift, Calendar, Mail, ArrowRight, Lock, Check
} from 'lucide-react';
import { useIsMobile } from '../../../hooks/use-is-mobile';
import { useWaitlist } from '../../../context/WaitlistContext';
import { VIEWPORT_ONCE } from '../../../lib/animation-performance';
// NOTE: relocated from features/landing/components → features/new-landing/sections.
// Both are 3 levels below client/src so the relative paths above are unchanged.
    ...
    return (
        <div className="flex items-center gap-1 font-mono text-2xl font-bold">
            {[0, 1].map(i => (
                <motion.span
                    key={`d${i}`}
                    className="w-8 h-10 flex items-center justify-center rounded-lg bg-[#4C82F7]/20 text-[#7FA8FF] gpu-stable"
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-12 gap-3 md:gap-5 w-full mx-auto relative z-10">
            ...
            <Perspective3D className="col-span-1 md:col-span-5">
                <div className="h-full group relative overflow-hidden rounded-3xl border border-white/10 bg-[#0a0f1d]/60 backdrop-blur-xl p-3 md:p-6 transition-all duration-500 hover:bg-[#111827]/80 hover:border-[#5EE6C4]/30 hover:shadow-2xl hover:shadow-[#5EE6C4]/20">
                    <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#5EE6C4]/5 blur-[80px] rounded-full group-hover:bg-[#5EE6C4]/10 transition-colors" style={GPU_STYLE} />

                    <div className="relative z-10 h-full flex flex-col justify-between" style={{ transformStyle: 'preserve-3d' }}>
                        <motion.div
                            className="p-2 md:p-3 w-fit rounded-2xl bg-white/10 border border-white/10 mb-4 md:mb-8 group-hover:border-[#5EE6C4]/30 transition-colors gpu-stable"
                            style={{ transform: 'translateZ(40px)' }}
                            animate={{ y: [0, -6, 0] }}
                            transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
                        >
                            <Lock className="w-5 h-5 md:w-8 md:h-8 text-white group-hover:text-[#5EE6C4] transition-colors" />
                        </motion.div>

                        <div style={{ transform: 'translateZ(20px)' }}>
                            <h4 className="text-sm md:text-2xl font-bold text-white mb-1 md:mb-2 group-hover:text-[#D9FBF1] transition-colors">Early Access</h4>
            ...
            <Perspective3D className="col-span-2 md:col-span-7">
                <div className="h-full group relative overflow-hidden rounded-3xl border border-white/10 bg-[#0a0f1d]/60 backdrop-blur-xl p-3 md:p-6 transition-all duration-500 hover:bg-[#111827]/80 hover:border-[#7FA8FF]/30 hover:shadow-2xl hover:shadow-[#7FA8FF]/20">
                    <div className="absolute bottom-0 right-0 w-64 h-64 bg-[#7FA8FF]/5 blur-[80px] rounded-full group-hover:bg-[#7FA8FF]/10 transition-colors" style={GPU_STYLE} />

                    <div className="relative z-10 h-full flex flex-col justify-between" style={{ transformStyle: 'preserve-3d' }}>
                        <div className="flex justify-between items-start mb-4 md:mb-8">
                            <motion.div
                                className="p-2 md:p-3 w-fit rounded-2xl bg-white/10 border border-white/10 mb-4 md:mb-8 group-hover:border-[#7FA8FF]/30 transition-colors gpu-stable"
                                style={{ transform: 'translateZ(40px)' }}
                                animate={{ y: [0, -6, 0] }}
                                transition={{ duration: 5.5, repeat: Infinity, ease: 'easeInOut', delay: 0.8 }}
                            >
                                <Mail className="w-5 h-5 md:w-8 md:h-8 text-white group-hover:text-[#7FA8FF] transition-colors" />
                            </motion.div>
                        </div>

                        <div style={{ transform: 'translateZ(20px)' }}>
                            <h4 className="text-lg md:text-2xl font-bold text-white mb-1 md:mb-2 group-hover:text-[#E3ECFF] transition-colors">Priority Support</h4>
                            <p className="text-xs md:text-base text-blue-100/80 font-medium leading-relaxed">
    ...
    return (
        <section id="beta-signup" className="py-24 relative overflow-hidden">
            <div className="absolute inset-0" style={GPU_STYLE}>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-gradient-radial from-[#4C82F7]/10 via-transparent to-transparent rounded-full blur-3xl" />
            </div>
                            ...
                            <Perspective3D className="w-full">
                                <motion.div
                                    ...
                                    style={{
                                        ...
                                        boxShadow: isFocused
                                            ? '0 0 60px -20px rgba(76,130,247,0.5)'
