...
export function AdaptiveAnimationProvider({
  ...
  useEffect(() => {
    ...
    // Only listen to connection changes, NOT resize
    // Resize events fire constantly on mobile (address bar hide/show) causing flickering
    connection?.addEventListener?.('change', handleConnectionChange);
    // REMOVED: window.addEventListener('resize', handleConnectionChange);

    return () => {
      connection?.removeEventListener?.('change', handleConnectionChange);
      // REMOVED: window.removeEventListener('resize', handleConnectionChange);
    };
