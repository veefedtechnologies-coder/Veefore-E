...
const Card = memo(({ feature, index, activeIndex }: { feature: Feature, index: number, activeIndex: MotionValue<number> }) => {
  ...
  return (
    ...
    >
      {/* Background gradient - blur disabled for mobile performance */}
      <div className={`absolute inset-0 bg-gradient-to-br ${feature.gradient} opacity-[0.12]`} />
      ...
      <div className="relative w-full h-full max-w-[1400px] mx-auto p-6 pt-20 md:p-12 md:pt-28 flex flex-col justify-center">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center h-full max-h-[800px]">
          ...
          <div className="relative w-full h-auto aspect-auto md:max-h-[60vh] order-1 lg:order-2 flex items-center justify-center z-10 mb-6 md:mb-0">
            {/* Visual container - backdrop blur works via CSS desktop-only */}
            <div className="relative w-full h-auto bg-gradient-to-tr from-white/10 to-white/5 rounded-xl md:rounded-[2rem] border border-white/10 md:backdrop-blur-xl overflow-hidden shadow-2xl p-0.5 md:p-1 transform transition-transform hover:scale-[1.02] duration-500">
            ...
            {/* Glows - desktop only with blur, mobile uses opacity gradients */}
            <div className={`absolute -top-10 -right-10 w-40 h-40 md:w-64 md:h-64 bg-gradient-to-br ${feature.gradient} rounded-full hidden md:block blur-[150px] opacity-40 pointer-events-none`} />
            <div className={`absolute -bottom-10 -left-10 w-32 h-32 md:w-48 md:h-48 bg-gradient-to-tr ${feature.gradient} rounded-full hidden md:block blur-[120px] opacity-30 pointer-events-none`} />
            {/* Mobile-only gradient orbs without blur */}
            <div className={`absolute -top-10 -right-10 w-40 h-40 bg-gradient-radial ${feature.gradient.replace('to-br', 'radial')} rounded-full md:hidden opacity-30 pointer-events-none`} />
          </div>
        </div>
      </div>
    </motion.div>
