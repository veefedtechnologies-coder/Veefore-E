...
/**
 * ScheduledPostsSkeleton — Page_Skeleton for `pages/ScheduledPostsPage.tsx`.
 *
 * Reproduces the real page's outer slot pixel-for-pixel: the
 * `min-h-full pb-16` wrapper, the premium gradient header band
 * (`mb-8 pb-8 border-b`) with a back-button + title + count pill + subtitle,
 * the `max-w-7xl mx-auto px-4 sm:px-6 lg:px-8` content container, and the
 * `grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6` post-card
 * grid built from `PostCardSkeleton`. The card count is clamped to [3, 10]
 * via `clampListCount` so the skeleton never implies the exact final count.
 * Pure and presentational — no data, no effects.
 *
 * Spec: pixel-perfect-skeleton-loading (R4.1, R5.1, R5.2, R5.3, R8.2, R8.3, R10.1).
 */
function ScheduledPostsSkeletonImpl() {
  const count = clampListCount(8, { default: 8 })

  return (
    <div data-testid="scheduled-posts-skeleton" className="min-h-full pb-16">
      {/* Premium header band */}
...
export const ScheduledPostsSkeleton = React.memo(ScheduledPostsSkeletonImpl)
ScheduledPostsSkeleton.displayName = 'ScheduledPostsSkeleton'
