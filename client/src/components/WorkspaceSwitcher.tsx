...
export default function WorkspaceSwitcher({ onNavigateToWorkspaces }: WorkspaceSwitcherProps) {
  ...
  // ✅ FIX: Re-read localStorage when any component (e.g. WorkspaceSelectionModal)
  // changes the active workspace externally and fires 'workspace-changed'.
  useEffect(() => {
    const handleExternalChange = () => {
      const updated = getSanitizedWorkspaceId();
      if (updated) setCurrentWorkspaceId(updated);
    };
  ...
  const handleCreateWorkspace = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const inactive = ((data?.data ?? data ?? []) as any[]).filter(
  ...
  const currentWorkspace =
    safeWorkspaces.find((ws: Workspace) =>
      currentWorkspaceId ? ws.id === currentWorkspaceId : ws.isDefault
    ) ||
    safeWorkspaces.find((ws: Workspace) => ws.isDefault) ||
    safeWorkspaces[0];
  ...
  const handleWorkspaceSwitch = async (workspaceId: string) => {
    ...
    if (!workspaceId || workspaceId === 'undefined' || workspaceId === 'null') {
      console.warn(
        '[WorkspaceSwitcher] ❌ Attempted to switch to invalid workspace ID:',
        workspaceId
      );
      return;
    }

    const workspace = safeWorkspaces.find((ws: any) => ws.id === workspaceId);
    if (!workspace) return;

    // PLAN ENFORCEMENT: block switching to locked (over-limit) workspaces.
    // The server annotates each workspace with `locked: true` when it exceeds
    // the user's current plan limit. Data is preserved — the user just can't
    // switch to it without upgrading.
    if ((workspace as any).locked) {
      toast({
        title: 'Workspace locked',
        description:
          (workspace as any).lockedReason ??
          'Upgrade your plan to access this workspace. Your data is safe.',
        variant: 'default',
        action: (
          <a
    ...
    <>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            className="flex items-center space-x-3 h-auto p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl"
          >
            <div
              className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getThemeGradient(currentWorkspace.theme)} flex items-center justify-center text-white shadow-sm`}
            >
            ...
            <div className="flex items-center space-x-2">
              <div className="text-left">
                <div className="flex items-center space-x-1">
                  ...
                  {currentWorkspace.isDefault && <Crown className="w-3 h-3 text-yellow-500" />}
                </div>
              ...
              return (
                <DropdownMenuItem
                  ...
                  className={`flex items-center space-x-3 p-3 cursor-pointer rounded-lg ${
                    isLocked
                      ? 'opacity-60 hover:bg-amber-50 dark:hover:bg-amber-900/20'
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center text-white shadow-sm flex-shrink-0 ${
                      isLocked
                        ...
                        : `bg-gradient-to-br ${getThemeGradient(workspace.theme)}`
                    }`}
                  >
                    {isLocked ? <Lock className="w-5 h-5" /> : <Building2 className="w-5 h-5" />}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2">
                      ...
                      {workspace.isDefault && !isLocked && (
                        <Crown className="w-3 h-3 text-yellow-500 flex-shrink-0" />
                      )}
                      {workspace.id === currentWorkspace?.id && !isLocked && (
                    ...
                    {isLocked ? (
                      <div className="text-xs text-amber-600 dark:text-amber-400 mt-0.5 font-medium">
                        🔒 Upgrade to access — data is preserved
                      </div>
                    ) : (
                    ...
                    {!isLocked && workspace.description && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-amber-500">
                  {safeWorkspaces.length}/{planLimit} used — upgrade to add more
                </div>
              </div>
            </DropdownMenuItem>
          ) : (
...
const getValidWorkspaceId = (workspaces: Workspace[], storedId: string | null): string | null => {
  ...
  // PLAN ENFORCEMENT: the active workspace must be an ACCESSIBLE (non-locked)
  // one. Locked workspaces (over the current plan limit) can't be worked in —
  // selecting one as active is what made the app appear "inside" a locked
  // workspace after a downgrade. Prefer the accessible pool; only fall back to
  // the full list if somehow every workspace is locked (never expected, since
  // the plan always keeps at least `maxWorkspaces` accessible).
  const accessible = workspaces.filter(ws => !(ws as any).locked);
  const pool = accessible.length > 0 ? accessible : workspaces;

  // Check if stored ID is valid, exists, and is accessible.
  const isValidId =
    storedId &&
    storedId !== 'undefined' &&
    storedId !== 'null' &&
    storedId !== '' &&
    pool.some(ws => ws.id === storedId);

  if (isValidId) return storedId;

  // Return default or first ACCESSIBLE workspace ID.
  const fallback = pool.find(ws => ws.isDefault) || pool[0];
...
export function useCurrentWorkspace() {
  ...
  useEffect(() => {
    ...
    const syncLocalStorage = async () => {
      ...
      if (currentStored === 'undefined' || currentStored === 'null' || currentStored === '') {
        console.warn(
          '[useCurrentWorkspace] 🧹 Removing invalid localStorage value:',
          currentStored
        );
      ...
      if (needsCorrection) {
        console.warn(
          '[useCurrentWorkspace] ❌ Invalid workspace ID in localStorage:',
          currentStored
        );
  ...
  useEffect(() => {
    ...
    if (workspaces.length === 0) {
      const autoCreate =
        (import.meta as any).env?.VITE_AUTO_CREATE_WORKSPACE === 'true' &&
      ...
      (async () => {
        try {
          ...
          if (created && created.id && created.id !== 'undefined') {
            ...
            console.warn(
              '[useCurrentWorkspace] ⚠️ Workspace creation returned invalid ID:',
              created?.id
            );
          }
          window.dispatchEvent(new Event('workspace-changed'));
