...
import { BestTimeWidgetSkeleton } from '@/components/skeletons'
import { Clock, TrendingUp, Sparkles, ArrowRight, Activity } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { useLocation } from 'wouter'
import { useBestTimeData } from '@/features/analytics/hooks/useBestTimeData'

/**
 * BestTimeWidget — dashboard home card showing the unified best-time
 * recommendation (audience-online + engagement + reach, from bestTimeEngine).
 * Reads the same `/api/v1/analytics/best-time` endpoint as the Analytics
 * "Best Time to Post" → Smart Pick tab, so the two never disagree.
 */
export function BestTimeWidget() {
  const [, setLocation] = useLocation()
  const { data, isLoading, isFetching } = useBestTimeData()
  ...
  if (!bestSlot) {
    // Still fetching in the background — avoid flashing the empty state
    if (isFetching) {
      return <BestTimeWidgetSkeleton />
    }
  ...
  const { confidence, confidenceLevel, meta } = smart!
    ...
    <Card className="relative overflow-hidden border-gray-200/50 dark:border-gray-700/50 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300 border-0 rounded-3xl group">
      ...
      <CardContent className="space-y-6 pt-2">
        ...
        <div className="flex items-end justify-between">
          <div>
            <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Peak Opportunity</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
             ...
             <div className="text-[10px] font-medium text-gray-400 dark:text-gray-500 mt-0.5">{confidenceLevel}</div>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
             <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Data Points</div>
             <div className="text-lg font-bold text-gray-900 dark:text-gray-100 leading-tight">
               {meta.usablePosts} Posts
             </div>
        ...
        {/* Action Button */}
        <Button
          onClick={() => setLocation('/analytics/best-time')}
          variant="outline"
