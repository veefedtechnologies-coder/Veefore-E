...
import { signInWithEmailAndPassword, signInWithRedirect, getRedirectResult, auth, googleProvider, sendPasswordResetEmail } from '@/lib/firebase'
...
const SignIn = ({ onNavigate }: SignInProps) => {
    ...
    if (!newErrors.email && !newErrors.password) {
      ...
      try {
        ...
        if (errorCode === 'auth/user-not-found') {
          ...
          userMessage = 'Incorrect password. If you signed up with Google, try "Continue with Google" instead.'
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Use redirect flow instead of popup to avoid popup blockers in Safari and other browsers
    // This provides a better cross-browser experience
    try {
      await signInWithRedirect(auth, googleProvider)
      // The redirect will happen immediately, no need to process result here
      // Result processing happens in the useEffect hook that calls getRedirectResult()
    } catch (error: any) {
      console.error('Google redirect sign in error:', error)
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="relative my-4">
                  ...
                  <div className="relative flex justify-center text-sm">
                    <span className="px-3 text-white/40 bg-black text-xs">or continue with</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full h-11 rounded-md bg-white/5 border border-white/10 text-white font-medium text-sm flex items-center justify-center gap-3 hover:bg-white/10 transition-colors disabled:opacity-70"
                >
