...
import {
  isBootstrapAuthed,
  isBootstrapOnboarded,
  isBootstrapCookied,
  hasAuthHint,
  isBootstrapExplicitlyLoggedOut,
} from './lib/bootstrap';
import { initializeTheme } from './lib/theme';
import { initializeP6System, P6Provider, ToastContainer } from './lib/p6-integration';
import {
  initializeAccessibilityCompliance,
  useAccessibilityRouteAnnouncements,
} from './lib/accessibility-compliance';
...
function App() {
  ...
  useEffect(() => {
    ...
    const disablePerfOpt = (import.meta as any).env?.VITE_DISABLE_PERF_OPT === 'true';
    ...
    const initAll = () => {
      ...
      if (disablePerfOpt) {
        console.warn(
          '[DIAGNOSTIC] VITE_DISABLE_PERF_OPT=true — skipping CoreWebVitals / ComponentModernization / P6 init'
        );
        return;
      }
  ...
  const isPublicRoute = useMemo(
    () =>
      publicRoutes.some(
        route => effectiveLocation === route || effectiveLocation.startsWith(route + '/')
      ),
    [effectiveLocation]
  );
  ...
  useEffect(() => {
    ...
    if (!loading && user && (effectiveLocation === '/signin' || effectiveLocation === '/signup')) {
      console.log(
        '[App] User is authenticated but on auth page, checking if this is intentional...'
      );
    ...
    if (
      ...
      (effectiveLocation === '/automation' || effectiveLocation === '/inbox')
    ) {
      setLocation('/');
    }
  }, [
  ...
  useEffect(() => {
    ...
    const onSignupResume = effectiveLocation === '/signup' && searchParams.get('resume') === 'true';
    ...
    const isMetaCallback =
      ...
      (searchParams.get('authorizedBrandCount') !== null ||
        searchParams.get('meta_error') !== null);
    if (isMetaCallback) return;

    console.log(
      '[App] User not onboarded — navigating to /signup?resume=true to complete onboarding'
    );
  ...
  const renderPublicPage = () => {
    switch (effectiveLocation) {
      ...
      case '/features':
        return (
          <RouteErrorBoundary routeName="Features">
            <React.Suspense fallback={null}>
              <Features />
            </React.Suspense>
          </RouteErrorBoundary>
      ...
      case '/security':
        return (
          <RouteErrorBoundary routeName="Security">
            <React.Suspense fallback={null}>
              <Security />
            </React.Suspense>
          </RouteErrorBoundary>
      ...
      case '/gdpr':
        return (
          <RouteErrorBoundary routeName="GDPR">
            <React.Suspense fallback={null}>
              <GDPR />
            </React.Suspense>
          </RouteErrorBoundary>
      ...
      case '/free-trial':
        return (
          <RouteErrorBoundary routeName="Free Trial">
            <React.Suspense fallback={null}>
              <FreeTrial />
            </React.Suspense>
          </RouteErrorBoundary>
      ...
      case '/landing':
        return (
          <RouteErrorBoundary routeName="New Landing">
            <React.Suspense fallback={null}>
              <NewLanding onNavigate={handleNavigate} />
            </React.Suspense>
          </RouteErrorBoundary>
          ...
          {user && effectiveLocation === '/signup' && isResumingOnboarding ? (
            ...
            <RouteErrorBoundary routeName="App">
              <React.Suspense fallback={<AppShellSkeleton pathname={effectiveLocation} />}>
                {effectiveLocation === '/subscription/manage' ? (
            ...
            effectiveLocation === '/landing' ? (
              ...
              <PublicPageLayout onNavigate={handleNavigate}>{renderPublicPage()}</PublicPageLayout>
            )
          ) : (
