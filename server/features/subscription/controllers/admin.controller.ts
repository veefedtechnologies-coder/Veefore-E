...
async function invalidateEntitlementCache(userId: string): Promise<void> {
  try {
    ...
    logger.warn('Failed to invalidate entitlement cache', {
      userId,
      err,
      module: 'admin.controller',
    });
...
async function recordAdminEvent(
  ...
  try {
    ...
    logger.error('Failed to write admin SubscriptionEvent audit record', err, {
      eventType,
      userId,
      subscriptionId,
      adminUserId,
...
// ---------------------------------------------------------------------------
// Helper — resolve the acting ADMIN's id for audit attribution
// ---------------------------------------------------------------------------

/**
 * These routes are protected by `requireAdminAuth` (see admin.routes.ts), which
 * attaches the authenticated admin record to `req.admin`. Older code read
 * `req.user?.id` here, which is only populated by the END-USER auth middleware
 * and was therefore always `undefined` under admin auth — silently writing
 * audit events with no attribution. Read `req.admin` first and fall back to
 * `req.user` so attribution is always recorded.
 */
function resolveAdminUserId(req: Request): string {
  const r = req as Request & {
    admin?: { id?: string | number };
    user?: { id?: string };
  };
  const adminId = r.admin?.id;
  if (adminId !== undefined && adminId !== null) return String(adminId);
  return r.user?.id ?? 'unknown';
}
...
export async function getUserSubscription(
  ...
  try {
    ...
    logger.error('getUserSubscription failed', err, {
      userId,
      module: 'admin.controller',
    });
 ...
 */
export async function setUserPlan(req: Request, res: Response): Promise<void> {
  const { userId } = req.params;
  ...
  try {
    ...
    if (!existing) {
      res
        .status(404)
        .json({ error: `No subscription found for userId: ${userId}` });
      return;
    }
    ...
    logger.info('Admin manually set user plan', {
      userId,
      adminUserId,
      previousPlan,
      newPlan: planId,
    ...
    logger.error('setUserPlan failed', err, {
      userId,
      adminUserId,
      module: 'admin.controller',
    });
...
export async function adjustCredits(
  ...
  try {
    ...
    if (!updated) {
      res
        .status(404)
        .json({ error: `No AI credits document found for userId: ${userId}` });
      return;
    }
    ...
    logger.info('Admin adjusted AI credits', {
      userId,
      adminUserId,
      amount,
      operation,
    ...
    logger.error('adjustCredits failed', err, {
      userId,
      adminUserId,
      module: 'admin.controller',
    });
...
export async function grantRevokeAddon(
  ...
  try {
    ...
    logger.info(`Admin ${action}ed add-on`, {
      userId,
      adminUserId,
      addonType,
      action,
    ...
    logger.error('grantRevokeAddon failed', err, {
      userId,
      adminUserId,
      module: 'admin.controller',
    });
...
export async function forceCancelSubscription(
  ...
  try {
    ...
    logger.error('forceCancelSubscription failed', err, {
      userId,
      adminUserId,
      module: 'admin.controller',
    });
...
export async function extendBillingPeriod(
  ...
  try {
    ...
    logger.info('Admin extended billing period', {
      userId,
      adminUserId,
      days,
      module: 'admin.controller',
    ...
    logger.error('extendBillingPeriod failed', err, {
      userId,
      adminUserId,
      module: 'admin.controller',
    });
...
export async function applyCoupon(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    logger.info('Admin applied coupon', {
      userId,
      adminUserId,
      couponCode,
      discountPercent,
...
export async function getSubscriptionHistory(
  ...
  try {
    const events = await SubscriptionEventModel.find({ userId })
      .sort({ timestamp: -1 })
      .limit(100)
      .lean();
    ...
    logger.error('getSubscriptionHistory failed', err, {
      userId,
      module: 'admin.controller',
    });
    res.status(500).json({ error: 'Internal server error' });
  ...
  try {
    ...
    if (!payment?.razorpayPaymentId) {
      res
        .status(404)
        .json({
          error: `No Razorpay payment found for paymentId: ${paymentId}`,
        });
      return;
    }
...
export async function setFeatureOverride(
  ...
  try {
    ...
    logger.info('Admin set feature override', {
      userId,
      adminUserId,
      featureKey,
      enabled,
    ...
    logger.error('setFeatureOverride failed', err, {
      userId,
      adminUserId,
      module: 'admin.controller',
    });
