...
import { tierAtLeast, type VeeGPTTier } from '../config/veegpt-tiers';
...
export interface VeeGPTAgent {
  ...
  icon: string;
  /**
   * Minimum VeeGPT tier this persona is available on. Personas that rely on
   * Full/Advanced tools (research, account analytics, growth insights) are not
   * offered to lower tiers, so we never advertise a capability the plan lacks.
   */
  minTier: VeeGPTTier;
...
export function agentPublicView(a: VeeGPTAgent): Omit<VeeGPTAgent, 'directives'> {
  return { id: a.id, name: a.name, description: a.description, icon: a.icon, minTier: a.minTier };
}
...
/** Whether an agent/persona is available on the given tier. */
export function isAgentAllowedForTier(id: string | null | undefined, tier: VeeGPTTier): boolean {
  return tierAtLeast(tier, getAgentById(id).minTier);
}

/** The agents available to the given tier (for the composer dropdown). */
export function agentsForTier(tier: VeeGPTTier): VeeGPTAgent[] {
  return VEEGPT_AGENTS.filter((a) => tierAtLeast(tier, a.minTier));
}
...
/**
 * Tier-aware directives: if the selected persona isn't available on the user's
 * tier, fall back to the default (no directives) so a crafted selectedAgentId
 * can't apply a higher-tier persona.
 */
export function getAgentDirectivesForTier(id: string | null | undefined, tier: VeeGPTTier): string {
  if (!isAgentAllowedForTier(id, tier)) return '';
  return getAgentDirectives(id);
}
