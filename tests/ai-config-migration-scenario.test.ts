...
describe('Task 8.4: Migration Scenario Test - Legacy to Workspace Config', () => {
  ...
  test('STEP 3: User saves new settings via fixed form', async () => {
    ...
    expect(updatedWorkspace?.aiConfiguration?.googleAiStudioKey).toBe('AI-zaSy_new_workspace_key_12345');
  ...
  test('STEP 5: AI generation uses workspace settings, NOT legacy user preferences', async () => {
    ...
    expect(apiKey).toBe('AI-zaSy_new_workspace_key_12345'); // From workspace, NOT 'LEGACY_KEY_WRONG_LOCATION'
    ...
    console.log('      - API Key: AI-zaSy_new_workspace_key_12345 (NOT LEGACY_KEY_WRONG_LOCATION)');
  ...
  test('STEP 6: Verify clean migration - workspace config overrides legacy', async () => {
    ...
    expect(workspace?.aiConfiguration?.googleAiStudioKey).toBe('AI-zaSy_new_workspace_key_12345');
  ...
  test('STEP 7: Form reload shows new workspace values (NOT legacy user preferences)', async () => {
    ...
    expect(formData?.googleAiStudioKey).toBe('AI-zaSy_new_workspace_key_12345'); // NOT 'LEGACY_KEY_WRONG_LOCATION'
