...
describe('Task 8.4: Migration Scenario Test - Legacy to Workspace Config', () => {
  ...
  const newWorkspaceConfiguration = {
    ...
    aiMemory: 'enabled',
    autoLearning: true,
    googleAiStudioKey: 'dummy_google_key',
  ...
  test('STEP 3: User saves new settings via fixed form', async () => {
    ...
    expect(updatedWorkspace?.aiConfiguration?.googleAiStudioKey).toBe('dummy_google_key');
  ...
  test('STEP 5: AI generation uses workspace settings, NOT legacy user preferences', async () => {
    ...
    expect(apiKey).toBe('dummy_google_key'); // From workspace, NOT 'LEGACY_KEY_WRONG_LOCATION'
    ...
    console.log('      - API Key: dummy_google_key (NOT LEGACY_KEY_WRONG_LOCATION)');
  ...
  test('STEP 6: Verify clean migration - workspace config overrides legacy', async () => {
    ...
    expect(workspace?.aiConfiguration?.googleAiStudioKey).toBe('dummy_google_key');
  ...
  test('STEP 7: Form reload shows new workspace values (NOT legacy user preferences)', async () => {
    ...
    expect(formData?.googleAiStudioKey).toBe('dummy_google_key'); // NOT 'LEGACY_KEY_WRONG_LOCATION'
