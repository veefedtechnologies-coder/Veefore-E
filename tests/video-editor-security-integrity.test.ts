...
const providerKeyArb: fc.Arbitrary<SecretCarrier> = fc
  .oneof(
    ...
    fc
      ...
      .map((c) => `dummy_google_key_${c.join('')}`),
    fc.array(fc.constantFrom(...UPPER_DIGIT), { minLength: 14, maxLength: 30 }).map((c) => `dummy_aws_key_${c.join('')}`),
    alnum(20, 30).map((t) => `dummy_github_key_${t}`),
    fc
