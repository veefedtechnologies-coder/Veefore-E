 ...
 * 
 * Extended middleware for Admin Panel with admin-specific features:
 ...
 * - Audit logging
 * 
 * This middleware extends shared authentication concepts while preserving
 * admin-specific JWT validation and security policies.
 * 
 ...
 * Validates JWT tokens and enforces admin-specific security policies
 */
 ...
 * Checks if admin has minimum required level (1-5, higher = more privileged)
 */
 ...
 * Automatically logs admin actions with risk assessment
 */
...
/**
 * IP whitelist checking middleware
 */
 ...
 * Enforces role-based time restrictions
 */
 ...
 * Enforces role-based IP restrictions
 */
 ...
 * Enforces data filtering based on role scope
 */
 ...
 * Enforces granular module-level permissions
 */
 ...
 * Marks sensitive actions that require higher-level approval
 */
...
/**
 * Combined RBAC middleware factory
 * Creates a middleware chain with all specified requirements
 * 
 * Usage:
 * router.get('/users', rbac({ roles: ['admin'], permissions: ['users.read'], level: 3 }), controller)
 */
...
// Backward compatibility with old auth.ts exports
export const authorize = (requiredRoles: string[]) => requireRole(requiredRoles);
